...
static int	wait_for_childs(int size, pid_t *pid_table)
{
	int	count;
	...
	while (count < size)
	{
		waitpid(pid_table[count], NULL, 0);
		count++;
	}
	return (0);
}
...
int	create_philo(t_data *data)
{
	...
	return (wait_for_childs(data->philo, pid_table));
}
